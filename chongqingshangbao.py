...
class mySpider(CrawlSpider):
    name = "chongqingshangbao"
    newspapers = "重庆商报"
    allowed_domains = ['e.chinacqsb.com']

    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y%m/%d")
        template = "https://e.chinacqsb.com/html/{date}/node_001.html"
        for d in dates:
            yield FormRequest(template.format(date = d))

    rules = (
        Rule(LinkExtractor(allow=('html/\d+/\d+/node_\w+.html'))),
        Rule(LinkExtractor(allow=('html/\d+/\d+/content\w+.html')), callback="parse_item")
    )
