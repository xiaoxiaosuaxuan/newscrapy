...
class mySpider(CrawlSpider):
    name = "fushunwanbao"
    newspapers = "抚顺晚报"
    allowed_domains = ['www.0245.net.cn']

    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y%m/%d")
        template = "http://www.0245.net.cn/fswbepaper/pc/paper/layout/{date}/node_wbad01.html"
        for d in dates:
            yield FormRequest(template.format(date=d))

    rules = (
        Rule(LinkExtractor(allow=('layout/\d+/\d+/node_wbad\w+.html'))),
