...
class mySpider(CrawlSpider):
    name = "huanghaichenkan"
    newspapers = "黄海晨刊"
    allowed_domains = ['epaper.rznews.cn']

    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y%m%d")
        template = "http://epaper.rznews.cn/shtml/hhcb/{date}/vA1.shtml"
        for d in dates:
            yield FormRequest(template.format(date = d))

    rules = (
        Rule(LinkExtractor(allow=('hhcb/\d+/vA\w+.shtml'))),
        Rule(LinkExtractor(allow=('hhcb/\d+/\w+.shtml')), callback="parse_item")
    )
    ...
    def parse_item(self, response):
        try:
            ...
            url = response.url
            date = re.search("hhcb/(\d+)/", url).group(1)
