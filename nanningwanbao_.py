...
class mySpider(CrawlSpider):
    name = "nanningwanbao"
    newspapers = "南宁晚报"
    allowed_domains = ['nnwb.nnnews.net']

    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y%m%d")
        template = "http://nnwb.nnnews.net:8080/nnwb/{date}/html/index.htm"
        for d in dates:
    ...
    rules = (
        ...
        Rule(LinkExtractor(allow=('\d+/html/index_\d+_content_\d+.htm')),callback='parse_item'),
    ...
    def parse_item(self, response):
        try:
            ...
            url = response.url
            date = re.search('nnwb/(\d+)/html', url).group(1)
