...
class mySpider(CrawlSpider):
    name = "tyrb"
    newspapers = "太原日报"
    allowed_domains = ['epaper.tyrbw.com']
    
    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y-%m/%d")
        template = "http://epaper.tyrbw.com/tyrb/html/{date}/node_1.htm"
        for d in dates:
    ...
    def parse_item(self, response):
        try:
            ...
            title1 = body.xpath(".//td[@class='font02']").xpath("string(.)").get()
            title2 = body.xpath(".//td[@class='font01']/founder-title").xpath("string(.)").get()
            title = title1+title2
