...
class mySpider(CrawlSpider):
    ...
    def start_requests(self):
        ...
        template = "https://szb.ahnews.com.cn/ahrb/layout/{date}/node_01.html"
        for d in dates:
            yield FormRequest(template.format(date = d))

    rules = (
        Rule(LinkExtractor(allow=('ahrb/layout/\d+\d+/\d+/node_\d+.html'))),
        Rule(LinkExtractor(allow=('ahrb/content/\d+\d+/\d+/c\d+.html')), callback="parse_item")
    )
