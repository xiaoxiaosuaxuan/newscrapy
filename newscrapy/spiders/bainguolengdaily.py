...
class mySpider(CrawlSpider):
    name = "bayinguolengdaily"
    newspapers = "巴音郭楞日报"
    allowed_domains = ['rb.loulannews.com']
    
    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y%m/%d")
        template = "http://rb.loulannews.com/pc/layout/{date}/node_01.html"
        for d in dates:
            yield FormRequest(template.format(date = d))

    rules = (
        Rule(LinkExtractor(allow=('pc/layout/\d+/\d+/node\w+.html'))),
        Rule(LinkExtractor(allow=('pc/cont/\d+/\d+/content\w+.html')), callback="parse_item")
    )
