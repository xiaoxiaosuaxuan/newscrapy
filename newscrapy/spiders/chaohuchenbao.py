...
class mySpider(CrawlSpider):
    ...
    def start_requests(self):
        ...
        template = "https://newspaper.hf365.com/chck/pc/layout/{date}/node_A01.html"
        for d in dates:
            yield FormRequest(template.format(date = d))

    rules = (
        Rule(LinkExtractor(allow=('chck/pc/layout/\d+\d+/\d+/node_A\d+.html'))),
        Rule(LinkExtractor(allow=('chck/pc/content/\d+\d+/\d+/content_\d+.html')), callback="parse_item")
    )
