...
class mySpider(CrawlSpider):
    name = "dahejiankangbao"
    newspapers = "大河健康报"
    allowed_domains = ['newpaper.dahe.cn']
    
    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y-%m/%d")
        template = "http://newpaper.dahe.cn/dhjkb/html/{date}/node_1221.htm"
        for d in dates:
            yield FormRequest(template.format(date = d))
#http://newpaper.dahe.cn/dhjkb/html/2022-08/23/node_1221.htm
#http://newpaper.dahe.cn/dhjkb/html/2022-08/23/content_589302.htm
    rules = (
