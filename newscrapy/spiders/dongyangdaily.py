...
class mySpider(CrawlSpider):
    name = "dongyangdaily"
    newspapers = "东阳日报"
    allowed_domains = ['dyrb.zjol.com.cn']

    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y-%m/%d")
        template = "http://dyrb.zjol.com.cn/html/{date}/node_2.htm"
        for d in dates:
    ...
    def parse_item(self, response):
        try:
            title = response.xpath("//td[@class='bt1']").xpath("string(.)").get()
            content = response.xpath("//td[@class='xilan_content_tt']").xpath('string(.)').get()
            url = response.url
