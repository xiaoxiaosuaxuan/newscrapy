...
class mySpider(CrawlSpider):
    name = "geermudaily"
    newspapers = "格尔木日报"
    allowed_domains = ['www.geermurb.com']
    
    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y-%m/%d")
        template = "http://www.geermurb.com/html/{date}/node_2.htm"
        for d in dates:
    ...
    def parse_item(self, response):
        try:
            ...
            content = response.xpath("//td[@class='font6']//founder-content").xpath("string(.)").get()
            url = response.url
