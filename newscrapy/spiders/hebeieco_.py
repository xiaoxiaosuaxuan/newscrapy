...
class mySpider(CrawlSpider):
    name = "hebeieco"
    newspapers = "河北经济网"
    allowed_domains = ['epaper.hbjjrb.com']

    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y%m/%d")
        template = "http://epaper.hbjjrb.com/{date}/node01.html"
        for d in dates:
            yield FormRequest(template.format(date=d))

    rules = (
        Rule(LinkExtractor(allow=('/\d+/\d+/node\w+.html'))),
        Rule(LinkExtractor(allow=('/\d+/\d+/con\w+.html')), callback="parse_item")
    )
    ...
    def parse_item(self, response):
        try:
            ...
            url = response.url
            date = re.search("/(\d+/\d+)/con", url).group(1)
