...
class mySpider(CrawlSpider):
    name = "jianghuaichenbao"
    newspapers = "江淮晨报"
    allowed_domains = ['newspaper.hf365.com']
    
    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y%m/%d")
        template = "https://newspaper.hf365.com/jhcb/pc/layout/{date}/node_A01.html"
        for d in dates:
            yield FormRequest(template.format(date = d))

    rules = (
        Rule(LinkExtractor(allow=('jhcb/pc/layout/\d+\d+/\d+/node_A\d+.html'))),
        Rule(LinkExtractor(allow=('jhcb/pc/content/\d+\d+/\d+/content_\d+.html')), callback="parse_item")
    )
