...
class mySpider(CrawlSpider):
    name = "jiatingyishengbao"
    newspapers = "家庭医生报"
    allowed_domains = ['jyb.ncrbw.cn']

    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y-%m/%d")
        template = "http://jyb.ncrbw.cn/html/{date}/node_73.htm"
        for d in dates:
    ...
    def parse_item(self, response):
        try:
            title = response.xpath("//div[@id='d_content']//h1").xpath("string(.)").get()
