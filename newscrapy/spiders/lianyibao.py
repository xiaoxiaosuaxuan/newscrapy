...
class mySpider(CrawlSpider):
    name = "lianyibao"
    newspapers = "联谊报"
    allowed_domains = ['www.lybs.com.cn']

    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y-%m/%d")
        template = "http://www.lybs.com.cn/html/{date}/node_1.htm"
        for d in dates:
    ...
    def parse_item(self, response):
        try:
            title = response.xpath("//div[@class='main-article-content']//h1").xpath("string(.)").get()
