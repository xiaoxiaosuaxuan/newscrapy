...
class mySpider(CrawlSpider):
    name = "loudidaily"
    newspapers = "娄底日报"
    allowed_domains = ['ldrb.ldnews.cn']

    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y/%m/%d")
        template = "http://ldrb.ldnews.cn/{date}/index.html"
        for d in dates:
            yield FormRequest(template.format(date = d))
#http://ldrb.ldnews.cn/2022/12/17/index.html
#http://ldwb.ldnews.cn/2022/12/30/index_2.html
#http://ldwb.ldnews.cn/2022/12/30/620384.html
    rules = (
