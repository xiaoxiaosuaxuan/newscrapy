...
class mySpider(CrawlSpider):
    name = "minbeidaily"
    newspapers = "闽北日报"
    allowed_domains = ['mbrb.greatwuyi.com']

    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y%m/%d")
        template = "https://mbrb.greatwuyi.com/{date}/node_1.html"
        for d in dates:
            yield FormRequest(template.format(date = d))

    rules = (
        Rule(LinkExtractor(allow=('\d+/\d+/node_\w+.html'))),
        Rule(LinkExtractor(allow=('/pc/content/\d+/\d+/content_\w+.html')), callback="parse_item")
    )
