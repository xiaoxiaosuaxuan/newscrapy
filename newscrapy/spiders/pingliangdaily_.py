...
class mySpider(CrawlSpider):
    name = "pingliangdaily"
    newspapers = "平凉日报"
    allowed_domains = ['szb.plxww.com']
    
    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y%m%d")
        template = "http://szb.plxww.com/epaper/htm/plrb/{date}/Page01BC.htm"
        for d in dates:
            yield FormRequest(template.format(date = d))

    rules = (
        Rule(LinkExtractor(allow=('\d+/Page01BC.htm'))),
    ...
    def parse_item(self, response):
        try:
            ...
            url = response.url
            date = re.search("plrb/(\d+)/Articel", url).group(1)
