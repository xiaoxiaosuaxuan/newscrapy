...
class mySpider(CrawlSpider):
    name = "sanqindushibao"
    newspapers = "三秦都市报"
    allowed_domains = ['epaper.sanqin.com']

    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y-%m/%d")
        template = "https://epaper.sanqin.com/html/{date}/node_1.htm"
        for d in dates:
            yield FormRequest(template.format(date = d))

    rules = (
       Rule(LinkExtractor(allow=('node.*\.htm'), restrict_xpaths="//div[@id='bmlistbar']")),
    ...
    def parse_item(self, response):
        try:
            ...
            imgs = response.xpath("//*[@id='newspic']//img/@src").getall()
