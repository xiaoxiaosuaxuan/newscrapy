...
class mySpider(CrawlSpider):
    name = "shangluodaily"
    newspapers = "商洛日报"
    allowed_domains = ['eslrb.slrbs.com']

    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y%m%d")
        template = "http://eslrb.slrbs.com/slrb/{date}/html/index.htm"
        for d in dates:
            yield FormRequest(template.format(date = d))

    rules = (
       Rule(LinkExtractor(allow=('index\.htm'), restrict_xpaths="//*[@id='titleList1']")),
       Rule(LinkExtractor(allow=('page.*\.htm'), restrict_xpaths="//*[@id='titleList1']")),
    ...
    def parse_item(self, response):
        try:
            ...
            url = response.url
            date = re.search('slrb/(\d{8})/', url).group(1)
