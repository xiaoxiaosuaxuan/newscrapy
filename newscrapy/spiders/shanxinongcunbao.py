...
class mySpider(CrawlSpider):
    name = "shanxinongcunbao"
    newspapers = "陕西农村报"
    allowed_domains = ['sxncb.joyhua.cn']

    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y%m%d")
        template = "http://sxncb.joyhua.cn/sxncb/{date}/html/index.htm"
        for d in dates:
    ...
    rules = (
       ...
       Rule(LinkExtractor(allow=('index_content.*\.htm')), callback="parse_item")
    )
    ...
    def parse_item(self, response):
        try:
            ...
            url = response.url
            date = re.search('sxncb/(\d{8})/', url).group(1)
