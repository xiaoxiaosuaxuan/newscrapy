...
class mySpider(CrawlSpider):
    ...
    def start_requests(self):    
        ...
        template = "http://epaper.shaoxing.com.cn/sxrb/pc/column/{date}/node_01.html"
        for d in dates:
            yield FormRequest(template.format(date = d))

    rules = (
        Rule(LinkExtractor(allow=('sxrb/pc/column/\d+/\d+/node\w+.html'))),
        Rule(LinkExtractor(allow=('sxrb/pc/content/\d+/\d+/content\w+.html')), callback="parse_item")
    )
