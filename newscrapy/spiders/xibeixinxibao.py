...
class mySpider(CrawlSpider):
    name = "xibeixinxibao"
    newspapers = "西北信息报"
    allowed_domains = ['xbxxb.joyhua.com']

    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y%m%d")
        template = "http://xbxxb.joyhua.com/xbxxb/{date}/html/index.htm"
        for d in dates:
    ...
    rules = (
       ...
       Rule(LinkExtractor(allow=('page.*content.*\.htm'), restrict_xpaths="//*[@class='bmdh_con']"), callback="parse_item")
    )
    ...
    def parse_item(self, response):
        try:
            ...
            url = response.url
            date = re.search('xbxxb/(\d{8})/', url).group(1)
