...
class mySpider(CrawlSpider):
    name = "xinwanbao"
    newspapers = "新晚报"
    allowed_domains = ['xwb.joyhua.cn']

    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y%m%d")
        template = "http://xwb.joyhua.cn/xwb/{date}/html/index.htm"
        for d in dates:
            yield FormRequest(template.format(date = d))
#http://xwb.joyhua.cn/xwb/20230128/html/index.htm
#http://xwb.joyhua.cn/xwb/20230128/html/page_01.htm
#http://xwb.joyhua.cn/xwb/20230128/html/content_20230128002001.htm
    rules = (
    ...
    def parse_item(self, response):
        try:
            ...
            url = response.url
            date = re.search('xwb/(\d+)/html', url).group(1)
