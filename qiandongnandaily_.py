...
class mySpider(CrawlSpider):
    name = "qiandongnandaily"
    newspapers = "黔东南日报"
    allowed_domains = ['dzb.qdnrbs.cn']

    def start_requests(self):
        dates = dateGen(self.start, self.end, "%Y%m/%d")
        template = "http://dzb.qdnrbs.cn/szb/pc/{date}/l01.html"
        for d in dates:
            yield FormRequest(template.format(date = d))

    rules = (
        Rule(LinkExtractor(allow=('pc/\d+/\d+/l\w+.html'))),
        Rule(LinkExtractor(allow=('pc/\d+/\d+/c\w+.html')), callback="parse_item")
    )
    ...
    def parse_item(self, response):
        try:
            ...
            url = response.url
            date = re.search("pc/(\d+/\d+)/", url).group(1)
